...
func (h *OpenAIAPIHandler) ImagesGenerations(c *gin.Context) {
	...
	if v := gjson.GetBytes(rawJSON, "output_compression"); v.Exists() && v.Type == gjson.Number {
		tool, _ = sjson.SetBytes(tool, "output_compression", v.Int())
	}
	if v := gjson.GetBytes(rawJSON, "partial_images"); v.Exists() && v.Type == gjson.Number {
...
func (h *OpenAIAPIHandler) ImagesEdits(c *gin.Context) {
	...
	c.JSON(http.StatusBadRequest, handlers.ErrorResponse{Error: handlers.ErrorDetail{Message: fmt.Sprintf("Invalid request: unsupported Content-Type %q", contentType), Type: "invalid_request_error"}})
}
...
func (h *OpenAIAPIHandler) imagesEditsFromJSON(c *gin.Context) {
	...
	if len(images) == 0 {
		c.JSON(http.StatusBadRequest, handlers.ErrorResponse{Error: handlers.ErrorDetail{Message: "Invalid request: images[].image_url is required (file_id is not supported)", Type: "invalid_request_error"}})
		return
	}
	...
	if mask := gjson.GetBytes(rawJSON, "mask.image_url"); mask.Exists() {
		...
		c.JSON(http.StatusBadRequest, handlers.ErrorResponse{Error: handlers.ErrorDetail{Message: "Invalid request: mask.file_id is not supported (use mask.image_url instead)", Type: "invalid_request_error"}})
		return
	}
...
func collectImagesFromResponsesStream(ctx context.Context, data <-chan []byte, errs <-chan *interfaces.ErrorMessage, responseFormat string) ([]byte, *interfaces.ErrorMessage) {
	...
	var cachedResults []imageCallResult
	var cachedCreatedAt int64
	var cachedUsageRaw []byte
	var cachedFirstMeta imageCallResult
	...
	processFrame := func(frame []byte) ([]byte, bool, *interfaces.ErrorMessage) {
		for _, line := range bytes.Split(frame, []byte("\n")) {
			...
			case "response.output_item.done":
				entry, ok := extractImageCallResultFromOutputItemDone(payload)
				if ok {
					if len(cachedResults) == 0 {
						cachedFirstMeta = entry
					}
					cachedResults = append(cachedResults, entry)
				}
			case "response.completed":
				createdAt, usageRaw, err := extractImagesCompletionMetadata(payload)
				if err != nil {
				...
				if len(usageRaw) > 0 {
					cachedUsageRaw = usageRaw
				}
				if len(cachedResults) == 0 {
					results, extractedCreatedAt, extractedUsageRaw, firstMeta, err := extractImagesFromResponsesCompleted(payload)
					if err != nil {
						return nil, false, &interfaces.ErrorMessage{StatusCode: http.StatusBadGateway, Error: err}
					}
					cachedResults = results
					if cachedCreatedAt == 0 {
						cachedCreatedAt = extractedCreatedAt
					}
					if len(cachedUsageRaw) == 0 {
						cachedUsageRaw = extractedUsageRaw
					}
					if len(cachedResults) > 0 {
						cachedFirstMeta = firstMeta
					}
				}
				if len(cachedResults) == 0 {
				...
				if cachedCreatedAt == 0 {
					cachedCreatedAt = time.Now().Unix()
				}
				out, err := buildImagesAPIResponse(cachedResults, cachedCreatedAt, cachedUsageRaw, cachedFirstMeta, responseFormat)
				if err != nil {
...
func extractImagesCompletionMetadata(payload []byte) (createdAt int64, usageRaw []byte, err error) {
	if gjson.GetBytes(payload, "type").String() != "response.completed" {
		return 0, nil, fmt.Errorf("unexpected event type")
	}
	createdAt = gjson.GetBytes(payload, "response.created_at").Int()
	if usage := gjson.GetBytes(payload, "response.tool_usage.image_gen"); usage.Exists() && usage.IsObject() {
		usageRaw = []byte(usage.Raw)
	}
	return createdAt, usageRaw, nil
}

func extractImageCallResultFromOutputItemDone(payload []byte) (imageCallResult, bool) {
...
func (h *OpenAIAPIHandler) forwardImagesStream(ctx context.Context, c *gin.Context, flusher http.Flusher, cancel func(error), data <-chan []byte, errs <-chan *interfaces.ErrorMessage, firstChunk []byte, responseFormat string, streamPrefix string, writeEvent func(string, []byte)) {
	...
	processFrame := func(frame []byte) (done bool) {
		for _, line := range bytes.Split(frame, []byte("\n")) {
			...
			case "response.completed":
				_, usageRaw, err := extractImagesCompletionMetadata(payload)
				if err != nil {
				...
				if len(results) == 0 {
					results, _, cachedUsageRaw, _, err = extractImagesFromResponsesCompleted(payload)
					if err != nil {
						emitError(&interfaces.ErrorMessage{StatusCode: http.StatusBadGateway, Error: err})
						return true
					}
				...
				for _, img := range results {
					...
					if len(cachedUsageRaw) > 0 && json.Valid(cachedUsageRaw) {
						data, _ = sjson.SetRawBytes(data, "usage", cachedUsageRaw)
					}
					writeEvent(eventName, data)
				}
				return true
